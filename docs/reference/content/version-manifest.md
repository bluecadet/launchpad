...
Node consumers connected to a running Launchpad daemon can instead renew the same lease through `content.ack`, and can read the active manifest without knowing its on-disk location through [`content.manifest.read`](./refetch-checker.md#manifest-discovery). See the [IPCClient consumer recipe](../../recipes/live-content-refresh.md#node-consumers-with-ipcclient).

Both commands are also reachable over plain HTTP via the [HTTP/SSE transport](../controller/transports.md), for browser and Unity/.NET consumers that can't open the IPC socket.
