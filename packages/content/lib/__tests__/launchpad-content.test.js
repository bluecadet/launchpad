...
describe('LaunchpadContent', () => {
	...
	/**
	 * @param {import('../content-plugin-driver.js').ContentPlugin[]} [plugins]
	 */
	...
	describe('plugin system', () => {
		it('should run plugins in correct order', async () => {
			...
			const plugin1 = {
				name: 'plugin1',
				hooks: {
					onContentFetchSetup: () => { order.push('plugin1:setup'); },
					onContentFetchDone: () => { order.push('plugin1:done'); }
				}
			};
			const plugin2 = {
				name: 'plugin2',
				hooks: {
					onContentFetchSetup: () => { order.push('plugin2:setup'); },
					onContentFetchDone: () => { order.push('plugin2:done'); }
				}
			};
	...
	describe('path handling', () => {
		it('should handle download path token replacement', () => {
			const content = new LaunchpadContent(createBasicConfig(), createMockLogger());
			const path = content._getDetokenizedPath(
				'/path/to/%DOWNLOAD_PATH%/file',
				'/downloads'
			);
