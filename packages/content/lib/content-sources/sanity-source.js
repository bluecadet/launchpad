...
class SanitySource extends ContentSource {
  ...
  async fetchContent() {
		...
		for (const query of this.config.queries) {
			await this._fetchPages(query, result, {
				start: 0,
  ...
  async _fetchPages(
		...
		const q = '*[_type == "' + query + '" ][' + params.start + '..' + (params.start + params.limit) + ']';
		const p = {};

		this.logger.debug(`Fetching page ${pageNum} of ${query}`);
		...
		return this.client.fetch(q, p).then((content) => {
			...
			const fileName = `${query}-${pageNum.toString().padStart(this.config.pageNumZeroPad, '0')}.json`;
			...
			if (this.config.maxNumPages < 0 || pageNum < this.config.maxNumPages - 1) {
				...
				return this._fetchPages(query, result, params);
			} else {
