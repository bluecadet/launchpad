import { createMockLogger, createMockPluginCtx } from "@bluecadet/launchpad-testing/test-utils.ts";
...
describe("sweepVersions", () => {
	...
	it("returns an empty result when versions/ does not exist yet", async () => {
		...
		const result = (
			await sweepVersions(DOWNLOAD_PATH, { keepVersions: 3, ackTimeout: ACK_TIMEOUT_MS }, logger)
		)._unsafeUnwrap();
	...
	it("extends retention for the manifest's active version even if older than keep-N", async () => {
		...
		const result = (
			await sweepVersions(DOWNLOAD_PATH, { keepVersions: 2, ackTimeout: ACK_TIMEOUT_MS }, logger)
		)._unsafeUnwrap();
	...
	it("extends retention for a fresh ack lease outside keep-N", async () => {
		...
		const result = (
			await sweepVersions(
				DOWNLOAD_PATH,
				{ keepVersions: 1, ackTimeout: ACK_TIMEOUT_MS },
				logger,
				now,
			)
		)._unsafeUnwrap();
	...
	it("honors an ack lease written by the content.ack command exactly like a hand-written one", async () => {
		for (const versionId of ["20260101T000000Z", "20260102T000000Z", "20260103T000000Z"]) {
			makeVersionDir(versionId);
		}
		...
		const ackResult = await instance.executeCommand({
			type: "content.ack",
			consumerId: "kiosk-1",
			versionId: "20260101T000000Z",
		});
		expect(ackResult).toBeOk();

		const logger = createMockLogger();
		const result = (
			await sweepVersions(DOWNLOAD_PATH, { keepVersions: 1, ackTimeout: ACK_TIMEOUT_MS }, logger)
		)._unsafeUnwrap();
	...
	it("keepVersions=0 retains exactly the active version and one fresh ack", async () => {
		makeVersionDir("20260101T000000Z"); // ack'd
		makeVersionDir("20260102T000000Z"); // neither active nor ack'd
		makeVersionDir("20260103T000000Z"); // active
		writeManifest("20260103T000000Z");
		const now = new Date("2026-07-14T15:35:00.000Z");
		writeAck("kiosk-1", "20260101T000000Z", now);
		const logger = createMockLogger();
		...
		const result = (
			await sweepVersions(
				DOWNLOAD_PATH,
				{ keepVersions: 0, ackTimeout: ACK_TIMEOUT_MS },
				logger,
				now,
			)
		)._unsafeUnwrap();
		...
		expect(result.retainedVersionIds.sort()).toEqual(["20260101T000000Z", "20260103T000000Z"]);
		expect(result.deletedVersionIds).toEqual(["20260102T000000Z"]);
		expect(vol.existsSync(`${getVersionsDirPath(DOWNLOAD_PATH)}/20260102T000000Z`)).toBe(false);
	});

	it("reaps a stale temp-manifest file but leaves a fresh one in place", async () => {
		vol.mkdirSync(DOWNLOAD_PATH, { recursive: true });
		const now = new Date("2026-07-14T15:35:00.000Z");

		const stalePath = `${DOWNLOAD_PATH}/.manifest.json.tmp-123-456-abcdef`;
		vol.writeFileSync(stalePath, "{}");
		const staleMtime = new Date(now.getTime() - 2 * 60 * 60 * 1000); // 2h old
		vol.utimesSync(stalePath, staleMtime, staleMtime);

		const freshPath = `${DOWNLOAD_PATH}/.manifest.json.tmp-789-012-ghijkl`;
		vol.writeFileSync(freshPath, "{}");
		vol.utimesSync(freshPath, now, now);

		const logger = createMockLogger();

		await sweepVersions(
			DOWNLOAD_PATH,
			{ keepVersions: 3, ackTimeout: ACK_TIMEOUT_MS },
			logger,
			now,
		);

		expect(vol.existsSync(stalePath)).toBe(false);
		expect(vol.existsSync(freshPath)).toBe(true);
	});
