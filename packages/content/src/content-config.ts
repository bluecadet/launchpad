import { z } from "zod";
import { contentPluginSchema, type ContentPlugin } from "./content-plugin-driver.js";
import { contentSourceSchema, type ContentSource } from "./sources/source.js";
...
export const contentConfigSchema = z.object({
	...
	sources: z.array(z.union([contentSourceSchema, z.promise(contentSourceSchema)])).describe("A list of content source options. This defines which content is downloaded from where.").default([]),
	...
	downloadPath: z.string().describe("The path at which to store all downloaded files. Defaults to '.downloads/'.").default(".downloads/"),
	/** Temp file directory path. Defaults to '.tmp/%TIMESTAMP/'. */
	tempPath: z.string().describe("Temp file directory path. Defaults to '.tmp/%TIMESTAMP/'.").default(".tmp/%TIMESTAMP%/"),
	/** Temp directory path where all downloaded content will be backed up before removal. Defaults to '.backup/%TIMESTAMP/'. */
	backupPath: z.string().describe("Temp directory path where all downloaded content will be backed up before removal. Defaults to '.backup/%TIMESTAMP/'.").default(".backup/%TIMESTAMP%/"),
	/** Which files to keep in `dest` if `clearOldFilesOnSuccess` or `clearOldFilesOnStart` are `true`. E.g. `['*.json', '** /*.csv', '*.xml', '*.git*']` */
	keep: z.array(z.string()).describe("Which files to keep in `dest` if `clearOldFilesOnSuccess` or `clearOldFilesOnStart` are `true`. E.g. `['*.json', '** /*.csv', '*.xml', '*.git*']`").default([]),
	/** Strips this string from all media file paths when saving them locally */
	strip: z.string().describe("Strips this string from all media file paths when saving them locally").default(""),
	/** Back up files before downloading and restore originals for all sources on failure of any single source. Defaults to true. */
	backupAndRestore: z.boolean().describe("Back up files before downloading and restore originals for all sources on failure of any single source. Defaults to true.").default(true),
	...
	encodeChars: z.string().describe("Characters to encode in the path when saving files locally. Defaults to `<>:\"|?*`. Applies to both content source paths and media download paths.").default("<>:\"|?*"),
});
