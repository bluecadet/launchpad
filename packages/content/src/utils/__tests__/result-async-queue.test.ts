...
describe("ResultAsyncQueue", () => {
	it("should add and process a single task", async () => {
		...
		const task = () => ResultAsync.fromPromise(Promise.resolve(42), () => new Error("Task failed"));
	...
	it("should process multiple tasks in order", async () => {
		...
		await Promise.all(tasks.map((task) => queue.add(task).map((value) => results.push(value))));
