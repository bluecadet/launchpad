/**
 * A never-throwing JSON serializer for logging and debugging output.
 *
 * `serializeJSON` always returns a string, even for values `JSON.stringify`
 * can't natively represent (bigint, Error, Map, Set, functions, symbols) or
 * would throw on (circular references). Unlike `IPCSerializer`, the output is
 * plain JSON meant for humans/log sinks, not a format designed to round-trip
 * back into the original value.
 */

const MAP_PLACEHOLDER = "[unserializable: map]";
const SET_PLACEHOLDER = "[unserializable: set]";
const FUNCTION_PLACEHOLDER = "[unserializable: function]";
const SYMBOL_PLACEHOLDER = "[unserializable: symbol]";
