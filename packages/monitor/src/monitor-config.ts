...
import { monitorPluginSchema, type MonitorPlugin } from "./core/monitor-plugin-driver.js";
import { z } from "zod";
...
const windowsApiConfigSchema = z.object({
	 ...
	 */
	nodeVersion: z.string().default(">=17.4.0").describe("The minimum major node version to support window ordering. Node versions < 17 seem to have a fatal bug with the native API, which will intermittently cause V8 to crash hard. Defaults to '>=17.4.0'.")
});
...
const windowConfigSchema = z.object({
	...
	foreground: z.boolean().default(false).describe("Move this app to the foreground once all apps have been launched. Defaults to false."),
	/** Minimize this app's windows once all apps have been launched. Defaults to false. */
	minimize: z.boolean().default(false).describe("Minimize this app's windows once all apps have been launched. Defaults to false."),
	...
	hide: z.boolean().default(false).describe("Completely hide this app's windows once all apps have been launched. Helpful for headless apps, but note that this might cause issues with GUI-based apps. Defaults to false.")
});
...
const appLogConfigSchema = z.object({
	...
	logToLaunchpadDir: z.boolean().default(true).describe("Route application logs to launchpad's log dir instead of pm2's log dir. Defaults to true."),
	...
	mode: logModesSchema.default("bus").describe("How to grab the app's logs. Supported values: - 'bus': Logs directly from the app's stdout/stderr bus. Can result in interrupted logs if the buffer isn't consistently flushed by an app. - 'file': Logs by tailing the app's log files. Slight lag, but can result in better formatting than bus. Not recommended, as logs cannot be rotated by launchpad. Defaults to 'bus'."),
	/** Whether or not to include output from `stdout`. Defaults to true. */
	showStdout: z.boolean().default(true).describe("Whether or not to include output from `stdout`. Defaults to true."),
	/** Whether or not to include output from `stderr`. Defaults to true. */
	showStderr: z.boolean().default(true).describe("Whether or not to include output from `stderr`. Defaults to true.")
});
...
const appConfigSchema = z.object({
	...
	}).passthrough().describe("pm2 configuration for this app. see https://pm2.keymetrics.io/docs/usage/application-declaration/#attributes-available") as z.ZodType<pm2.StartOptions & { out_file?: string; error_file?: string }>,
	/** Optional settings for moving this app's main windows to the foreground, minimize or hide them. */
	windows: windowConfigSchema.describe("Optional settings for moving this app's main windows to the foreground, minimize or hide them.").default({}),
	/** Optional settings for how to log this app's output. */
	logging: appLogConfigSchema.describe("Optional settings for how to log this app's output.").default({})
});
...
export const monitorConfigSchema = z.object({
	...
	deleteExistingBeforeConnect: z.boolean().default(false).describe("Set this to true to delete existing PM2 processes before connecting. If you're running volatile apps or your node process might be quit unexpectedly, this can be helpful to start with a clean slate on startup."),
	/** Advanced configuration for the Windows API, e.g. for managing foreground/minimized/hidden windows. */
	windowsApi: windowsApiConfigSchema.describe("Advanced configuration for the Windows API, e.g. for managing foreground/minimized/hidden windows.").default({}),
	/** A list of plugins. */
	plugins: z.array(monitorPluginSchema).default([]).describe("A list of plugins."),
	/** Will listen for exit events. Defaults to true. */
	shutdownOnExit: z.boolean().default(true).describe("Will listen for exit events. Defaults to true.")
});
