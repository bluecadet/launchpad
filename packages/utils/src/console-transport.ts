...
export const TTY_ONLY: unique symbol = Symbol('ttyOnly');
export const NO_TTY: unique symbol = Symbol('noTTY');
export const TTY_FIXED: unique symbol = Symbol('ttyFixed');
export const TTY_FIXED_END: unique symbol = Symbol('ttyFixedEnd');
...
export class FilterLogType implements Format {
	...
	transform(info: TransformableInfo) {
		...
		info[TTY_FIXED] = undefined;
		info[TTY_FIXED_END] = undefined;
...
export abstract class FixedConsoleLogger {
  ...
  constructor(protected logger: Logger, level = 'info', ttyUpdateInterval = 100) {
    this.#interval = setInterval(() => {
      const consoleMessage = this.getFixedConsoleMessage();
			logger.log({
				level,
